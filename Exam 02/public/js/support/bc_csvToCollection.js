  ...
  if(lector.result != null){
    ...
    for (let index = 1; index < data.length; index++) {
      ...
      for (let index2 = 0; index2 < etiquetas.length; index2++) {
        // Convierte el valor al formato nativo
        salida[etiquetas[index2]] = convertirAFormatoNatito(valores[index2]);
      }
...
function convertirAFormatoNatito(valor) {
  // Convierte a entero
  if (/^-?\d+$/.test(valor)) {
  ...
  // Si no es un número ni una fecha, devuelve "na"
  return "na";
}
